  ...
  useEffect(() => {
    ...
    if (tokenFromUrl) {
      ...
      if (stored) {
        ...
        fetch(`http://127.0.0.1:8888/refresh?refresh_token=${storedRefresh}`)
          .then(res => res.json())
          .then(data => {
...
//   useEffect(() => {
//   const devToken = import.meta.env.VITE_APPLE_DEVELOPER_TOKEN;
//   const appleUserToken = localStorage.getItem('apple_user_token');

//   if (!appleUserToken && devToken) {
//     loginToAppleMusic(devToken).then(music => {
//       console.log("Apple Music authorized:", music);
//     });
//   }
// }, []);
...
useEffect(() => {
  ...
  fetchOnce(); // 👈 run immediately on mount
